  ...
  public:
    ...
    void inline update(PyThreadState* tstate, FrameStack::Key stack, size_t size)
    {
        ...
        if (stack_entry == map.end()) {
            ...
            // the deallocations.
            map.emplace(stack,
                        MemoryStats(tstate->interp->id, thread_info_map[tstate->thread_id]->name, stack, 1, size));
        } else {
...
static PyMemAllocatorEx echion_allocator = { NULL, echion_malloc, echion_calloc, echion_realloc, echion_free };
