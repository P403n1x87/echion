...
void WhereRenderer::render_frame(Frame& frame)
{
    ...
    if (!name_str.has_value())
        return;  // Skip this frame if name lookup failed
    auto filename_str = string_table.lookup(frame.filename);
    if (!filename_str.has_value())
    ...
    {
        WhereRenderer::get().render_message("\033[33;1m" + name_str.value() + "\033[0m (\033[36m" +
                                            filename_str.value() + "\033[0m:\033[32m" +
