...
Result<std::string> pyunicode_to_utf8(PyObject* str_addr)
{
    PyUnicodeObject str;
...
[[nodiscard]] Result<StringTable::Key> StringTable::key(PyObject* s)
{
...
[[nodiscard]] StringTable::Key StringTable::key_unsafe(PyObject* s)
{
    const std::lock_guard<std::mutex> lock(table_lock);
...
[[nodiscard]] StringTable::Key StringTable::key(unw_word_t pc)
{
    const std::lock_guard<std::mutex> lock(table_lock);
...
[[nodiscard]] Result<StringTable::Key> StringTable::key(unw_cursor_t& cursor)
{
    const std::lock_guard<std::mutex> lock(table_lock);
...
[[nodiscard]] Result<std::string*> StringTable::lookup(Key key)
{
    const std::lock_guard<std::mutex> lock(table_lock);
...
StringTable::StringTable() : std::unordered_map<uintptr_t, std::string>()
{
    this->emplace(0, "");
